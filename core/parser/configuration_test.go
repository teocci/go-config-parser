...
const (
	ConfigFilePath       = "config_parser_test.ini"
	ConfigFilePathSHA    = "b28cf57c4f5a261d2e6810466c0f157392fe11d5"
	ConfigNewFilePath    = "config_parser_test_new.ini"
...
func TestSHA(t *testing.T) {
	out, err := exec.Command("sha-sum", ConfigNewFilePath).Output()
	if err != nil {
		t.Fatal(err)
	}
	...
	if sha[0] != ConfigNewFilePathSHA {
		t.Error(ConfigNewFilePath + " sha-sum does not match!")
	}
}
